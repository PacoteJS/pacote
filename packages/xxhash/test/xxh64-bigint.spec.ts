...
import { xxh64BigInt } from '../src/index'
...
describe('XXHash64', () => {
  test('default seed value is 0', () => {
    const h1 = xxh64BigInt().update(sanityBuffer.toString()).digest('hex')
    const h2 = xxh64BigInt(0).update(sanityBuffer.toString()).digest('hex')
    expect(h2).toBe(h1)
  })
      ...
      property(
        ...
        (seed, data) => {
          const referenceHash = new XXHash64(seed)
            ...
            .toString('hex')
          const hash = xxh64BigInt(seed).update(data).digest('hex')
  ...
  test.each([
    ...
    const actual = xxh64BigInt(seed).update(data.buffer).digest('hex')
  ...
  test('chunked updates', () => {
    assert(
      property(nat(), string({ maxLength: 256 }), (seed, data) => {
        const hashFromWhole = xxh64BigInt(seed).update(data).digest('hex')
        const hashFromChunks = data
          .split('')
          .reduce(
            (hasher, element) => hasher.update(element),
            xxh64BigInt(seed),
          )
          .digest('hex')
